        ...
        if (validUser == -1) {
            ...
            if (!validChannel) {
                ...
                if (num <= 0) {
                    ...
                    interaction.editReply({embeds: [{ description: `Invalid command usage: ${num} is not a valid transaction number.` }]});
                } else {
                    ...
                    (async function() {
                        handleDelete(interaction, interaction.user.id, transactionids[num - 1], recipients, num - 1)
                        .then((result) => {
                            if (result === 1) {
                                transactionid = transactionids[num-1].transactionid
                                db.run(`DELETE FROM transactions WHERE serverid = ? AND transactionid = ?;`, [interaction.guildId, transactionid]).then(() => {
                                    updateLog(interaction.guild);
                                });
                                db.run(`DELETE FROM transactionhands WHERE serverid = ? AND transactionid = ?;`, [interaction.guildId, transactionid]);
                            }
                        });
                    })();
                }
...
async function handleDelete(interaction, authorid, transaction, recipients, number) {
