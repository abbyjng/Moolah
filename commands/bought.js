...
const { checkValidUser, checkTransactionsChannel } = require('./../permissionHandler.js');
...
module.exports = {
	...
	async execute(interaction) {
        ...
        let validUser = await checkValidUser(interaction.user.id, interaction.guildId);
        if (validUser == -1) {
            interaction.editReply({embeds: [{ description: `No users are set. Set up users using \`/setUser [@user] [emoji]\`.` }]});
        } else if (validUser == 1) {
            let validChannel = await checkTransactionsChannel(interaction.channelId, interaction.guildId);
            if (!validChannel) {
                if (cost <= 0) {
                    interaction.editReply({embeds: [{ description: `Invalid command usage: the value submitted must be a positive value.` }]});
                } else if (description === "defaultPaidDescription") {
                    interaction.editReply({embeds: [{ description: `Congrats! You've found the one description message you are not allowed to use. Please try again.` }]});
                } else if (description && description.length > 200) {
                    interaction.editReply({embeds: [{ description: `Invalid command usage: the description submitted must be <200 characters long.` }]});
                } else {
                    sql = `SELECT userid, emoji FROM users WHERE serverid = ? AND status = 1`;
                    users = await db.all(sql, [interaction.guildId]);
                    (async function() {
                        handleTransaction(interaction, interaction.user.id, users, getFormattedUsers(users), cost, description)
                        .then((recipients) => {
                            if (recipients[0] !== 0 && recipients[0] !== -1) {
                                sql = `INSERT INTO transactions (serverid, value, description)
                                        VALUES (?, ?, ?);`;
                                db.run(sql, [interaction.guildId, cost/recipients.length, description]).then(() => {
                                    db.run("SELECT last_insert_rowid()").then((transactionid) => {
                                        recipients.forEach((recipient) => {
                                            sql = `INSERT INTO transactionhands (serverid, transactionid, owner, recipient)
                                                    VALUES (?, ?, ?, ?);`;
                                            db.run(sql, [interaction.guildId, transactionid.lastID, interaction.user.id, recipient.userid]).then(() => {
...
async function handleTransaction(interaction, authorid, users, strUsers, value, description) {
                ...
                if (user.id === authorid) {
                    if (reaction.emoji.name === '✅') {
                        ...
                        if (t[(m.createdAt, authorid)].recipients.length == 1 && t[(m.createdAt, authorid)].recipients[0].userid == authorid) {
                            ...
                            confirmTransaction(interaction, interaction.user.id, t[(m.createdAt, authorid)].recipients, value, description);
...
function confirmTransaction(interaction, ownerid, recipients, value, description) {
