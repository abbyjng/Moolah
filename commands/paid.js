        ...
        if (validUser == -1) {
            ...
            if (!validChannel) {
                if (cost <= 0) {
                    ...
                    (async function() {
                        handlePayment(interaction, interaction.user.id, users, getFormattedUsers(users, interaction.user.id), cost)
                        .then((recipient) => {
                    ...
                    if (!result) {
                        interaction.editReply({embeds: [{ description: `User not found: ${emoji} is not associated with any user in this server.` }]});
                    } else if (result.userid === interaction.user.id) {
                        interaction.editReply({embeds: [{ description: `Why are you trying to pay yourself? That's not allowed` }]});
                    } else {
                        ...
                        db.run(sql, [interaction.guildId, cost]).then(() => {
                            db.run("SELECT last_insert_rowid()").then((transactionid) => {
                                sql = `INSERT INTO transactionhands (serverid, transactionid, owner, recipient)
                                        VALUES (?, ?, ?, ?);`;
                                db.run(sql, [interaction.guildId, transactionid.lastID, interaction.user.id, userid]).then(() => {
                                    updateLog(interaction.guild);
                                });
                            });
                        });
                        confirmPayment(interaction, interaction.user.id, userid, emoji, cost);
                    }
                }
            } else {
