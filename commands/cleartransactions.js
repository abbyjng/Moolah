...
module.exports = {
	...
	async execute(interaction) {
        ...
        if (validUser == -1) {
            ...
            if (!validChannel) {
                (async function() {
                    handleClear(interaction, interaction.user.id)
                    .then((result) => {
                        if (result === 1) {
                            db.run(`DELETE FROM transactions WHERE serverid = ?;`, [interaction.guildId]).then(() => {
                                updateLog(interaction.guild);
                            });
                            db.run(`DELETE FROM transactionhands WHERE serverid = ?;`, [interaction.guildId]);
                        }
                    });
                })();
            } else {
