...
module.exports = {
	...
	async execute(interaction) {
		...
		if (!result) {
			interaction.editReply({embeds: [{ description: `User could not be removed. <@!${user.id}> is not currently in the database.` }]});
		} else {
            emojis = ['❌', '✅'];
    
            let embed = new Discord.MessageEmbed()
                .setTitle(`Remove this user?`)
                .setDescription(`Removing user <@!${user.id}> remove them from the log and not allow them to create new transactions. They will still appear in the transaction log.\n`)
            ...
            .then((m) => {
                ...
                collector.on('end', collected => {
                    ...
                    if (collected.length === 0) {
                        ...
                        db.run(`UPDATE users SET status = 0 WHERE userid = ? AND serverid = ?;`, [user.id, interaction.guildId]).then(() => {
